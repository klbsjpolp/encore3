...
import { clearRuntimeConfigCache, fetchRuntimeConfig } from './runtimeConfig'

// The opaque game identifier the relay server uses to route rooms. The server
// never interprets it; it only keeps games from sharing a room code space.
export const ENCORE_GAME_ID = 'encore'
...
const getApiBaseUrl = async (): Promise<string> => {
  const envApiBaseUrl = normalizeApiBaseUrl(import.meta.env.VITE_ENCORE_API_URL)
...
export const createOnlineRoom = async (playerName?: string): Promise<CreateRoomResponse> => {
  const request: CreateRoomRequest = { gameId: ENCORE_GAME_ID, playerName }
