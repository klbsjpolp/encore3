...
export function validateBoard(board: BoardConfiguration): { valid: boolean; errors: string[] } {
  ...
  // Count colors
  const colorCounts = Object.fromEntries(GAME_COLORS.map((color) => [color, 0])) as Record<
    GameColor,
    number
  >
  ...
  for (const color of GAME_COLORS) {
    ...
    if (JSON.stringify(sizes) !== JSON.stringify(expectedSizes)) {
      errors.push(
        `Color ${color} must have groups of sizes [1,2,3,4,5,6], found [${sizes.join(',')}]`,
      )
    }
  }
  ...
  const starsByColor = Object.fromEntries(GAME_COLORS.map((color) => [color, 0])) as Record<
    GameColor,
    number
  >
  ...
  if (starColumns.size !== 15) {
    errors.push(
      `Each column must have exactly one star, found stars in ${starColumns.size} columns`,
    )
  }
  ...
  for (const color of GAME_COLORS) {
    ...
    for (const k of starCounts) {
      if (k > 1) {
        errors.push(
          `Color ${color} must not have more than 1 star in a single group, found ${k} stars in one group`,
        )
        break
      }
    }
