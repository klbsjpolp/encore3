...
function ColumnPoint({ index, isClaimedByMe, isClaimedByOther, display, compact = false }: ColumnPointProps) {
  return (
    <div className={cn(
    ...
    >
      <div className={cn('grid grid-cols-15', compact ? 'gap-px' : 'gap-1')}>
        {COLUMNS.map((col, colIndex) => (
          <ColumnPoint key={col} index={colIndex} isClaimedByMe={false} isClaimedByOther={false} display={col} compact={compact} />
        ))}
      </div>

      <div className={cn(compact ? 'space-y-0 mt-px mb-px' : 'space-y-1 mt-1 @lg:mt-3 mb-1 @lg:mb-3')}>
        {board.map((row, rowIndex) => (
          <div key={rowIndex} className={cn('grid grid-cols-15', compact ? 'gap-0' : 'gap-0.5 @lg:gap-1')}>
            {row.map((square, colIndex) => (
              <button
                ...
                className={cn(
                  ...
                  compact ? 'rounded-[3px] border' : 'rounded-[3px] @lg:rounded-md border-1 @lg:border-2',
                  ...
                  isSquareHovered(rowIndex, colIndex) && (compact ? 'z-10 ring-1 ring-primary/70' : 'scale-105 shadow-md'),
                  isSquarePreview(rowIndex, colIndex) && (compact ? 'ring-2 ring-primary/70' : 'ring-2 ring-primary/50 scale-105'),
                  isStartingColumn(colIndex) && !isSquareSelected(rowIndex, colIndex) ? (compact ? 'outline-solid outline-1 outline-border' : 'outline-solid outline-1 @lg:outline-2 outline-border') : '',
              ...
              >
                {square.hasStar ? (
                  <Star
                    fill="white"
                    className={cn(
                      compact ? 'absolute inset-0 m-auto w-full h-full max-h-3 max-w-3' : 'absolute inset-0 m-auto w-full h-full max-h-6 max-w-6',
                      square.crossed ? 'text-muted-foreground' : 'text-black drop-shadow-md'
                    )}
                  />
                ) : (
                  <div className={cn('absolute opacity-25 bg-white rounded-full', compact ? 'inset-[1px]' : 'inset-[0.5px] @lg:inset-0.25 w-11/12 h-11/12')} />
                )}
                {square.crossed && (
      ...
      {showColumnScores && (
        <div className={cn('grid grid-cols-15', compact ? 'gap-px' : 'gap-1')}>
          {COLUMN_FIRST_PLAYER_POINTS.map((points, index) => {
            ...
            return <ColumnPoint index={index} key={index} isClaimedByMe={isClaimedByMe} isClaimedByOther={isClaimedByOther} display={points} compact={compact} />;
          })}
          {COLUMN_SECOND_PLAYER_POINTS.map((points, index) => {
            const isClaimedByMe = iClaimedSecondBonus.includes(COLUMNS[index]);
            return <ColumnPoint index={index} key={index} isClaimedByMe={isClaimedByMe} isClaimedByOther={false} display={points} compact={compact} />;
          })}
        </div>
      )}
    </div>
