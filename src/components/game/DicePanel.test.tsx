...
import type { ColorDiceResult, DiceResult } from '@/types/game'
...
const mockSelectedColorDie: ColorDiceResult = {
  id: 'c1',
  type: 'color',
  value: 'red',
  selected: false,
...
describe('DicePanel Component', () => {
  it('renders the dice panel with roll button', () => {
    render(<DicePanel dice={mockDice} phase="rolling" canRoll={true} />)
  ...
  it('calls onRollDice when roll button is clicked', () => {
    ...
    render(<DicePanel dice={mockDice} phase="rolling" canRoll={true} onRollDice={onRollDice} />)
  ...
  it('disables roll button when canRoll is false', () => {
    render(<DicePanel dice={mockDice} phase="active-selection" canRoll={false} />)
  ...
  it('disables selection when canSelect is false', () => {
    const onDiceSelect = vi.fn()
    render(
      <DicePanel dice={mockDice} phase="rolling" canSelect={false} onDiceSelect={onDiceSelect} />,
    )
  ...
  it('renders compact dice in a single ordered row', () => {
    render(<DicePanel dice={mockDice} phase="active-selection" canSelect={true} compact={true} />)
  ...
  it('animates dice on the first roll transition', () => {
    ...
    const { rerender } = render(<DicePanel dice={[]} phase="rolling" canRoll={true} />)

    rerender(<DicePanel dice={mockDice} phase="active-selection" canSelect={true} />)
    ...
    expect(Array.from(rollingColorDice[0].classList).some((c) => c.startsWith('animate-'))).toBe(
      true,
    )
